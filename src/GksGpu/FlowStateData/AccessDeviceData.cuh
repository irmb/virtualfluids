...
__host__ __device__ inline void readCellDataUpdate(const uint cellIdx, const DataBaseStruct& dataBase, ConservedVariables& cellCons, const char* dir)
{
    real* dataUpdate = nullptr;
    
    if(dir == "px") dataUpdate = dataBase.dataUpdatePX;
    if(dir == "py") dataUpdate = dataBase.dataUpdatePY;
    if(dir == "pz") dataUpdate = dataBase.dataUpdatePZ;
    if(dir == "mx") dataUpdate = dataBase.dataUpdateMX;
    if(dir == "my") dataUpdate = dataBase.dataUpdateMY;
    if(dir == "mz") dataUpdate = dataBase.dataUpdateMZ;

    cellCons.rho  = dataUpdate[ RHO__( cellIdx, dataBase.numberOfCells ) ];
    cellCons.rhoU = dataUpdate[ RHO_U( cellIdx, dataBase.numberOfCells ) ];
    cellCons.rhoV = dataUpdate[ RHO_V( cellIdx, dataBase.numberOfCells ) ];
    cellCons.rhoW = dataUpdate[ RHO_W( cellIdx, dataBase.numberOfCells ) ];
    cellCons.rhoE = dataUpdate[ RHO_E( cellIdx, dataBase.numberOfCells ) ];
#ifdef USE_PASSIVE_SCALAR
	cellCons.rhoS_1 = dataUpdate[ RHO_S_1( cellIdx, dataBase.numberOfCells ) ];
	cellCons.rhoS_2 = dataUpdate[ RHO_S_2( cellIdx, dataBase.numberOfCells ) ];
#endif // USE_PASSIVE_SCALAR
}

__host__ __device__ inline void writeCellDataUpdate(const uint cellIdx, const DataBaseStruct& dataBase, ConservedVariables& cellCons, const char* dir)
{
    real* dataUpdate = nullptr;
    ...
    dataUpdate[ RHO__( cellIdx, dataBase.numberOfCells ) ] = cellCons.rho ;
    dataUpdate[ RHO_U( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoU;
    dataUpdate[ RHO_V( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoV;
    dataUpdate[ RHO_W( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoW;
    dataUpdate[ RHO_E( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoE;
#ifdef USE_PASSIVE_SCALAR
	dataUpdate[ RHO_S_1( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoS_1;
	dataUpdate[ RHO_S_2( cellIdx, dataBase.numberOfCells ) ] = cellCons.rhoS_2;
#endif // USE_PASSIVE_SCALAR
}
