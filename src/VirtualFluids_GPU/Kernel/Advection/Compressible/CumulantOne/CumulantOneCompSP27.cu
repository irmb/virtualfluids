...
std::shared_ptr<Kernel> CumulantOneCompSP27::getNewInstance(std::shared_ptr<Parameter> para, int level)
{
	return std::shared_ptr<Kernel>(new CumulantOneCompSP27(para, level));
}
...
void CumulantOneCompSP27::run()
{
	...
	LB_Kernel_Kum_New_Comp_SP_27 << < grid, threads >> >(	para->getParD(level)->omega,
...
bool CumulantOneCompSP27::checkParameter()
{
	if (para->getUseWale())
		return false;
