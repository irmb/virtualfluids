...
public:
	...
	void VIRTUALFLUIDS_GPU_EXPORT writeInit(SPtr<Parameter> para, SPtr<CudaMemoryManager> cudaManager) override;
	//! \brief write time step to VTK file(s)
	//! \param para instance of classParameter
	//! \param timestep of the simulation
	void VIRTUALFLUIDS_GPU_EXPORT writeTimestep(SPtr<Parameter> para, uint timestep) override;
