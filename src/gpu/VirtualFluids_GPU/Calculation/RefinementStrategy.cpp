...
void RefinementAndExchange_streams_exchangeInterface::operator()(UpdateGrid27 *updateGrid, Parameter *para, int level)
{
    ...
    //!
    updateGrid->fineToCoarse(level, &para->getParD(level)->intFCBorder, para->getParD(level)->neighborFC, CudaStreamIndex::SubDomainBorder);
    ...
    updateGrid->fineToCoarse(level, &para->getParD(level)->intFCBulk, para->getParD(level)->neighborFCBulk, CudaStreamIndex::SubDomainBorder);
    updateGrid->coarseToFine(level, &para->getParD(level)->intCFBulk, para->getParD(level)->neighborCFBulk, CudaStreamIndex::SubDomainBorder);
    ...
    //!
    updateGrid->coarseToFine(level, &para->getParD(level)->intCFBorder, para->getParD(level)->neighborCF, CudaStreamIndex::SubDomainBorder);
