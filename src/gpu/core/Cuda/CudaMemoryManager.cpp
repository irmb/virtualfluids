                                                 ...
                                                 ProcessNeighbor27& recvNeighborHost, ProcessNeighbor27& recvNeighborDevice)
{
    ...
    checkCudaErrors(cudaMallocHost((void**)&(sendNeighborHost.f[0]), sendNeighborHost.memsizeFs));
    checkCudaErrors(cudaMallocHost((void**)&(recvNeighborHost.index), recvNeighborHost.memsizeIndex));
    checkCudaErrors(cudaMallocHost((void**)&(recvNeighborHost.f[0]), recvNeighborHost.memsizeFs));

    // Device
    checkCudaErrors(cudaMalloc((void**)&(sendNeighborDevice.index), sendNeighborDevice.memsizeIndex));
    checkCudaErrors(cudaMalloc((void**)&(sendNeighborDevice.f[0]), sendNeighborDevice.memsizeFs));
    checkCudaErrors(cudaMalloc((void**)&(recvNeighborDevice.index), recvNeighborDevice.memsizeIndex));
    checkCudaErrors(cudaMalloc((void**)&(recvNeighborDevice.f[0]), recvNeighborDevice.memsizeFs));
    ...
    {
        checkCudaErrors(cudaMallocHost((void**)&(sendNeighborHost.fAD[0]), sendNeighborHost.memsizeFs));
        checkCudaErrors(cudaMallocHost((void**)&(recvNeighborHost.fAD[0]), recvNeighborHost.memsizeFs));
        checkCudaErrors(cudaMalloc((void**)&(sendNeighborDevice.fAD[0]), sendNeighborDevice.memsizeFs));
        checkCudaErrors(cudaMalloc((void**)&(recvNeighborDevice.fAD[0]), recvNeighborDevice.memsizeFs));
...
void CudaMemoryManager::cudaFreeProcessNeighbor(ProcessNeighbor27& sendNeighborHost, ProcessNeighbor27& sendNeighborDevice, ProcessNeighbor27& recvNeighborHost, ProcessNeighbor27& recvNeighborDevice)
{
    ...
    checkCudaErrors(cudaFreeHost(sendNeighborHost.f[0]));
    checkCudaErrors(cudaFreeHost(recvNeighborHost.index));
    checkCudaErrors(cudaFreeHost(recvNeighborHost.f[0]));
    checkCudaErrors(cudaFree(sendNeighborDevice.index));
    checkCudaErrors(cudaFree(sendNeighborDevice.f[0]));
    checkCudaErrors(cudaFree(recvNeighborDevice.index));
    checkCudaErrors(cudaFree(recvNeighborDevice.f[0]));
    if(parameter->getDiffOn())
    {
        checkCudaErrors(cudaFreeHost(sendNeighborHost.fAD[0]));
        checkCudaErrors(cudaFreeHost(recvNeighborHost.fAD[0]));
        checkCudaErrors(cudaFree(sendNeighborDevice.fAD[0]));
        checkCudaErrors(cudaFree(recvNeighborDevice.fAD[0]));
    }
}
...
void CudaMemoryManager::cudaCopyProcessNeighborFsHtoD(ProcessNeighbor27* neighborHost, ProcessNeighbor27* neighborDevice)
{
    if (!parameter->getStreamManager()->streamIsRegistered(CudaStreamIndex::SubDomainBorder)) {
        checkCudaErrors(cudaMemcpy(neighborDevice->f[0], neighborHost->f[0], neighborDevice->memsizeFs, cudaMemcpyHostToDevice));
        if (parameter->getDiffOn())
            checkCudaErrors(cudaMemcpy(neighborDevice->fAD[0], neighborHost->fAD[0], neighborDevice->memsizeFs, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpyAsync(neighborDevice->f[0], neighborHost->f[0], neighborDevice->memsizeFs,cudaMemcpyHostToDevice,parameter->getStreamManager()->getStream(CudaStreamIndex::SubDomainBorder)));
        if (parameter->getDiffOn()) {
            checkCudaErrors(cudaMemcpyAsync(neighborDevice->fAD[0], neighborHost->fAD[0], neighborDevice->memsizeFs,cudaMemcpyHostToDevice,parameter->getStreamManager()->getStream(CudaStreamIndex::SubDomainBorder)));
        }
    }
}
...
{
    if (!parameter->getStreamManager()->streamIsRegistered(CudaStreamIndex::SubDomainBorder)) {
        checkCudaErrors(cudaMemcpy(neighborHost->f[0], neighborDevice->f[0], neighborDevice->memsizeFs, cudaMemcpyDeviceToHost));
        if (parameter->getDiffOn())
            checkCudaErrors(cudaMemcpy(neighborHost->fAD[0], neighborDevice->fAD[0], neighborDevice->memsizeFs, cudaMemcpyDeviceToHost));
    } else {
        checkCudaErrors(cudaMemcpyAsync(neighborHost->f[0], neighborDevice->f[0], neighborDevice->memsizeFs,cudaMemcpyDeviceToHost,parameter->getStreamManager()->getStream(CudaStreamIndex::SubDomainBorder)));
        if (parameter->getDiffOn())
            checkCudaErrors(cudaMemcpyAsync(neighborHost->fAD[0], neighborDevice->fAD[0], neighborDevice->memsizeFs,cudaMemcpyDeviceToHost,parameter->getStreamManager()->getStream(CudaStreamIndex::SubDomainBorder)));
    }
}
...
void CudaMemoryManager::cudaAllocStressBC(int lev)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    double tmp = 2 * memSizeInt + parameter->getD3Qxx() * memSizeReal + 9 * memSizeReal;
    setMemsizeGPU(tmp, false);
}
...
void CudaMemoryManager::cudaAllocSurfaceLayerBC(int lev)
{
    ...
    checkCudaErrors( cudaMallocHost((void**) &(parameter->getParH(lev)->surfaceLayerBC.kN),                           memSizeInt     ));
    ...
    checkCudaErrors( cudaMalloc((void**) &(parameter->getParD(lev)->surfaceLayerBC.kN),                                memSizeInt    ));
    ...
    //////////////////////////////////////////////////////////////////////////
    const double tmp = static_cast<double>(2*memSizeInt + parameter->getD3Qxx()*memSizeReal + 9*memSizeReal);
    setMemsizeGPU(tmp, false);
}
...
void CudaMemoryManager::cudaCopySurfaceLayerBC(int lev)
{
    ...
    checkCudaErrors( cudaMemcpy(parameter->getParD(lev)->surfaceLayerBC.kN,      parameter->getParH(lev)->surfaceLayerBC.kN,                            memSizeInt,       cudaMemcpyHostToDevice));
...
void CudaMemoryManager::cudaFreeSurfaceLayerBC(int lev)
{
    ...
    checkCudaErrors( cudaFreeHost(parameter->getParH(lev)->surfaceLayerBC.kN));
    ...
    checkCudaErrors( cudaFree(parameter->getParD(lev)->surfaceLayerBC.kN));
...
// Wall model
void CudaMemoryManager::cudaAllocWallModel(WallModelParameters& wallModelHost, WallModelParameters& wallModelDevice, uint numberOfNodes, bool hasWallModelMonitor)
{
    const uint memSizeInt = sizeof(int) *  numberOfNodes;
    const uint memSizeReal = sizeof(real) *  numberOfNodes;

    //Host
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.samplingIndices), memSizeInt));
    ...
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityNodeX), memSizeReal));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityNodeY), memSizeReal));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityNodeZ), memSizeReal));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityExchangeLocationX), memSizeReal));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityExchangeLocationY), memSizeReal));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.velocityExchangeLocationZ), memSizeReal));
    ...
    // Device
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.samplingIndices), memSizeInt));
    ...
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityNodeX), memSizeReal));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityNodeY), memSizeReal));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityNodeZ), memSizeReal));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityExchangeLocationX), memSizeReal));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityExchangeLocationY), memSizeReal));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.velocityExchangeLocationZ), memSizeReal));
    ...
    //////////////////////////////////////////////////////////////////////////
    double tmp = double(memSizeInt+9*memSizeReal);
    if(hasWallModelMonitor)
        tmp += double(4*memSizeReal);
    setMemsizeGPU(tmp, false);
}
void CudaMemoryManager::cudaCopyWallModel(WallModelParameters& wallModelHost, WallModelParameters& wallModelDevice, uint numberOfNodes)
{
    ...
    checkCudaErrors(cudaMemcpy(wallModelDevice.samplingIndices, wallModelHost.samplingIndices, memSizeInt, cudaMemcpyHostToDevice));
...
void CudaMemoryManager::cudaFreeWallModel(WallModelParameters& wallModelHost, WallModelParameters& wallModelDevice, bool hasWallModelMonitor)
{
    ...
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityNodeX));
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityNodeY));
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityNodeZ));
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityExchangeLocationX));
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityExchangeLocationY));
    checkCudaErrors(cudaFreeHost(wallModelHost.velocityExchangeLocationZ));
    ...
    checkCudaErrors(cudaFree(wallModelDevice.velocityNodeX));
    checkCudaErrors(cudaFree(wallModelDevice.velocityNodeY));
    checkCudaErrors(cudaFree(wallModelDevice.velocityNodeZ));
    checkCudaErrors(cudaFree(wallModelDevice.velocityExchangeLocationX));
    checkCudaErrors(cudaFree(wallModelDevice.velocityExchangeLocationY));
    checkCudaErrors(cudaFree(wallModelDevice.velocityExchangeLocationZ));
                                                      ...
                                                      TemperatureWallModelParameters& wallModelDevice, uint numberOfNodes)
{
    ...
    // Host
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.temperatureExchangeLocation), memSize));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.temperatureFirstFluidNode), memSize));
    ...
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.temperatureScale), memSize));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.roughnessLength), memSize));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.invObukhovLength), memSize));
    checkCudaErrors(cudaMallocHost((void**)&(wallModelHost.heatingRate), memSize));

    // Device
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.temperatureExchangeLocation), memSize));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.temperatureFirstFluidNode), memSize));
    ...
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.temperatureScale), memSize));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.roughnessLength), memSize));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.invObukhovLength), memSize));
    checkCudaErrors(cudaMalloc((void**)&(wallModelDevice.heatingRate), memSize));

    //////////////////////////////////////////////////////////////////////////
    const double tmp = static_cast<double>(9 * memSize);
    setMemsizeGPU(tmp, false);
}
                                                     ...
                                                     TemperatureWallModelParameters& wallModelDevice, uint numberOfNodes)
{
    ...
    checkCudaErrors(cudaMemcpy(wallModelDevice.temperatureExchangeLocation, wallModelHost.temperatureExchangeLocation, memSize,
                               cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(wallModelDevice.temperatureFirstFluidNode, wallModelHost.temperatureFirstFluidNode, memSize,
                               cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(wallModelDevice.surfaceHeatFlux, wallModelHost.surfaceHeatFlux, memSize, cudaMemcpyHostToDevice));
    checkCudaErrors(
        cudaMemcpy(wallModelDevice.surfaceTemperature, wallModelHost.surfaceTemperature, memSize, cudaMemcpyHostToDevice));
    ...
    checkCudaErrors(cudaMemcpy(wallModelDevice.invObukhovLength, wallModelHost.invObukhovLength, memSize, cudaMemcpyHostToDevice));
...
{
    checkCudaErrors(cudaFreeHost(wallModelHost.temperatureExchangeLocation));
    checkCudaErrors(cudaFreeHost(wallModelHost.temperatureFirstFluidNode));
    ...
    checkCudaErrors(cudaFreeHost(wallModelHost.roughnessLength));
    checkCudaErrors(cudaFreeHost(wallModelHost.invObukhovLength));
    checkCudaErrors(cudaFreeHost(wallModelHost.heatingRate));

    checkCudaErrors(cudaFree(wallModelDevice.temperatureExchangeLocation));
    checkCudaErrors(cudaFree(wallModelDevice.temperatureFirstFluidNode));
    ...
    checkCudaErrors(cudaFree(wallModelDevice.roughnessLength));
    checkCudaErrors(cudaFree(wallModelDevice.invObukhovLength));
