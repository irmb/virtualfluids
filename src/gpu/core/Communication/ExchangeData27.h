...
void copyEdgeNodes(const std::vector<LBMSimulationParameter::EdgeNodePositions>& edgeNodes,
                   ...
                   const std::vector<ProcessNeighbor27>& sendProcessNeighborsHost, bool diffOn);
...
//! \param const CudaMemoryManager is needed for moving the data between host and device
...
//! respective edges
void exchangeCollDataGPU27(
    Parameter* para, vf::parallel::Communicator& comm, const CudaMemoryManager* cudaMemoryManager,
    CudaStreamIndex streamIndex, const std::vector<ProcessNeighbor27>& sendProcessNeighborsDevice,
    const std::vector<ProcessNeighbor27>& recvProcessNeighborsDevice,
...
void exchangeCollDataXGPU27AllNodes(Parameter* para, vf::parallel::Communicator& comm,
                                    const CudaMemoryManager* cudaMemoryManager, int level, CudaStreamIndex streamIndex);
