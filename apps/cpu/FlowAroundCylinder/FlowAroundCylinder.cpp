    ...
    try {
        ...
        if (logToFile) {
            ...
            if (myid == 0) {
                ...
                logFilename << pathOut + "/logfile" + ub_system::toString(ub_system::getTimeStamp()) + ".txt";
        ...
        // SPtr<MPIIOMigrationSimulationObserver> restart(new MPIIOMigrationSimulationObserver(grid, mSch, metisVisitor,
        // pathOut, comm));
        SPtr<MPIIORestartSimulationObserver> restart =
            make_shared<MPIIORestartSimulationObserver>(grid, mSch, pathOut, comm);
        ...
        SPtr<GbObject3D> cylinder(
            new GbCylinder3D(0.5, 0.2 - L2 * 0.5, -0.1 - L3 * 0.5, 0.5, 0.2 - L2 * 0.5, L3 + 0.1 - L3 * 0.5, radius));
        gb_system_3d::writeGeoObject(cylinder.get(), pathOut + "/geo/cylinder", WbWriterVtkXmlBinary::getInstance());

        SPtr<D3Q27Interactor> cylinderInt =
            SPtr<D3Q27Interactor>(new D3Q27Interactor(cylinder, grid, noSlipAdapter, Interactor3D::SOLID));
        ...
        if (myid == 0)
            gb_system_3d::writeGeoObject(addWallYmin.get(), pathOut + "/geo/addWallYmin",
        ...
        if (myid == 0)
            gb_system_3d::writeGeoObject(addWallYmax.get(), pathOut + "/geo/addWallYmax",
                                         WbWriterVtkXmlASCII::getInstance());

        GbCuboid3DPtr addWallZmin(new GbCuboid3D(g_minX1 - blockLength, g_minX2 - blockLength, g_minX3 - blockLength,
                                                 g_maxX1 + blockLength, g_maxX2 + blockLength, g_minX3));
        if (myid == 0)
            gb_system_3d::writeGeoObject(addWallZmin.get(), pathOut + "/geo/addWallZmin",
                                         WbWriterVtkXmlASCII::getInstance());

        GbCuboid3DPtr addWallZmax(new GbCuboid3D(g_minX1 - blockLength, g_minX2 - blockLength, g_maxX3,
                                                 g_maxX1 + blockLength, g_maxX2 + blockLength, g_maxX3 + blockLength));
        if (myid == 0)
            gb_system_3d::writeGeoObject(addWallZmax.get(), pathOut + "/geo/addWallZmax",
                                         WbWriterVtkXmlASCII::getInstance());
        ...
        SPtr<D3Q27Interactor> inflowInt =
            SPtr<D3Q27Interactor>(new D3Q27Interactor(geoInflow, grid, velBC, Interactor3D::SOLID));

        // outflow
        SPtr<D3Q27Interactor> outflowInt =
            SPtr<D3Q27Interactor>(new D3Q27Interactor(geoOutflow, grid, denBC, Interactor3D::SOLID));
        ...
        if (newStart) {
            ...
            // SPtr<GbObject3D> refCylinder(new GbCylinder3D(0.5, 0.2-L2*0.5, -0.1-L3*0.5, 0.5, 0.2-L2*0.5, L3+0.1-L3*0.5,
            // radius+7.0*dx/(1<<refineLevel)));
            SPtr<GbObject3D> refCylinder(new GbCuboid3D(0.32, g_minX2 - blockLength, g_minX3 - blockLength, 0.68,
                                                        g_maxX2 + blockLength, g_maxX3 + blockLength));
            gb_system_3d::writeGeoObject(refCylinder.get(), pathOut + "/geo/refCylinder",
                                         WbWriterVtkXmlBinary::getInstance());

            // inflow
            GbCuboid3DPtr refInflow(new GbCuboid3D(g_minX1 - blockLength, g_minX2 - blockLength, g_minX3 - blockLength,
                                                   g_minX1 + blockLength, g_maxX2 + blockLength, g_maxX3 + blockLength));
            if (myid == 0)
                gb_system_3d::writeGeoObject(geoInflow.get(), pathOut + "/geo/refInflow",
                                             WbWriterVtkXmlASCII::getInstance());

            // outflow
            GbCuboid3DPtr refOutflow(new GbCuboid3D(g_maxX1 - blockLength, g_minX2 - blockLength, g_minX3 - blockLength,
                                                    g_maxX1 + blockLength, g_maxX2 + blockLength, g_maxX3 + blockLength));
            if (myid == 0)
                gb_system_3d::writeGeoObject(geoOutflow.get(), pathOut + "/geo/refOutflow",
                                             WbWriterVtkXmlASCII::getInstance());

            SPtr<GbObject3D> gridCube(new GbCuboid3D(g_minX1, g_minX2, g_minX3, g_maxX1, g_maxX2, g_maxX3));
            if (myid == 0)
                gb_system_3d::writeGeoObject(gridCube.get(), pathOut + "/geo/gridCube", WbWriterVtkXmlBinary::getInstance());
            ...
            SPtr<SimulationObserver> ppblocks(new WriteBlocksSimulationObserver(
                grid, SPtr<UbScheduler>(new UbScheduler(1)), pathOut, WbWriterVtkXmlBinary::getInstance(), comm));
            ...
            SPtr<SimulationObserver> ppgeo(new WriteBoundaryConditionsSimulationObserver(
                grid, geoSch, pathOut, WbWriterVtkXmlBinary::getInstance(), comm));
            ppgeo->update(0);
            ppgeo.reset();
        ...
        SPtr<SimulationObserver> writeMQSimulationObserver(new WriteMacroscopicQuantitiesSimulationObserver(
            grid, stepSch, pathOut, WbWriterVtkXmlBinary::getInstance(), conv, comm));
        writeMQSimulationObserver->update(0);

        real area = (2.0 * radius * H) / (dx * dx);
        real v = 4.0 * uLB / 9.0;
        SPtr<UbScheduler> forceSch(new UbScheduler(1000));
        SPtr<CalculateForcesSimulationObserver> fp =
            make_shared<CalculateForcesSimulationObserver>(grid, forceSch, pathOut + "/forces/forces.csv", comm, v, area);
        fp->addInteractor(cylinderInt);

        SPtr<UbScheduler> nupsSch(new UbScheduler(nupsStep[0], nupsStep[1], nupsStep[2]));
        std::shared_ptr<SimulationObserver> nupsSimulationObserver(
