...
if use_refinement:
    ...
    grid_builder.add_grid(
        gpu.grid_generator.Cuboid(
            ...
            0,
            turbine_positions_x[0] + 10 * turbine_diameter,
            turbine_positions_y[0] + 3 * turbine_diameter,
            turbine_positions_z[0] + 3 * turbine_diameter,
        ),
        level,
    )
...
if read_precursor:
    bc_factory.set_precursor_boundary_condition(
        gpu.PrecursorBC.DistributionsPrecursor if use_distributions else gpu.PrecursorBC.VelocityPrecursor
    )
...
# %%
planar_average_probe = gpu.probes.PlanarAverageProbe(
    para,
    cuda_memory_manager,
    "horizontalPlanes",
...
# %%
wall_model_probe = gpu.probes.WallModelProbe(
    para,
    cuda_memory_manager,
    "wallModelProbe",
    para.get_output_path(),
